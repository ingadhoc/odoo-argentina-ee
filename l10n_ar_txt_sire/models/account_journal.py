...
class AccountJournal(models.Model):
    ...
    def sire_files_values(self, move_lines):
        ...
        for line in move_lines.sorted(key=lambda r: (r.date, r.id)):
            ...
            content += payment.tax_withholding_id.codigo_regimen.zfill(3)
            ...
            domicilio = " ".join(item for item in [
            ...
            content += line.partner_id.sire_born_country_id.l10n_ar_afip_code if es_persona and line.partner_id.sire_born_country_id else ' ' * 3
            # 29 Retenido nacimiento constitucion fecha (date 10, 406-415, no obligatorio)
            content += line.partner_id.sire_birthdate.strftime('%d/%m/%Y') if es_persona and line.partner_id.sire_birthdate else ' ' * 10
            content += '\r\n'
        ...
        for line in move_lines.sorted(key=lambda r: (r.date, r.id)):
            ...
            if not payment.tax_withholding_id.codigo_regimen:
                raise RedirectWarning(
                    message=_("El impuesto '%s' no tiene código de régimen establecido. Es obligatorio para generar el"
                              " archivo txt Sire. Editar campo 'Codigo de regimen IVA' en solapa 'Opciones avanzadas'"
                              " en la vista formulario", payment.tax_withholding_id.name),
                    action=payment.tax_withholding_id.get_formview_action(),
                    button_text=_('Editar impuesto'),
                )

            # Validamos que el partner sea Cliente / Proveedor del Exterior
            if line.partner_id.l10n_ar_afip_responsibility_type_id.id not in [self.env.ref('l10n_ar.res_EXT').id,
                                                                              self.env.ref('l10n_ar.res_EXT_Prov').id]:
                raise RedirectWarning(
                    message=_("Solo puede generar el archivo de retenciones SIRE para contactos con responsabilidad"
            ...
            if not payment.sire_aplica_cdi and int(payment.sire_codigo_alicuota) > 83:
                raise UserError(_("El pago %s (id: %s) debe tener código de alícuota"
                                  " menor a 83 ya que no aplica CDI" % (payment.name, payment.id)))
    ...
    def certificado_retencion_impositiva_files_values(self, move_lines):
        ...
        for line in move_lines.sorted(key=lambda r: (r.date, r.id)):
            ...
            if not payment.tax_withholding_id.codigo_regimen:
                raise RedirectWarning(
                    message=_("El impuesto '%s' no tiene código de régimen establecido."
            ...
            content += '%06.2f' % payment.tax_withholding_id.porcentaje_exclusion\
            if payment.tax_withholding_id.porcentaje_exclusion != '0.0' else '000.00'
