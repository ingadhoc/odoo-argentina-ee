...
class AccountJournal(models.Model):
    ...
    def sire_files_values(self, move_lines):
        ...
        for line in move_lines.sorted(key=lambda r: (r.date, r.id)):
            ...
            # 1 Formulario (integer long 4, 1-4, obligatorio) --> '2003' comentado por agi en tarea 40050, en nota del
            # 23/10/2024 a las 10:00hs
            # Además en la sección "3.2. F2003 - Validaciones", pàgina 5 del pdf de la especificación se indica que
            # debe ser fijo "2003"
            content += '2003'
            # 2 Versión (integer long 4, 5-8, obligatorio) --> '0100' comentado por agi en tarea 40050, en nota del
            # 23/10/2024 a las 10:00hs
            ...
            content += payment.tax_withholding_id.codigo_regimen[:3].zfill(3)
    ...
    def _sire_validations(self, move_lines):
        ...
        for line in move_lines.sorted(key=lambda r: (r.date, r.id)):
            ...
            # Validamos que el cuit de la compañía sea de 11 dígitos
            company_vat = payment.company_id.vat
            if not company_vat or (company_vat and len(company_vat) != 11):
                raise RedirectWarning(
                    message=_("El cuit de la compañía '%s' debe ser de 11 dígitos de longitud.", payment.company_id.name),
                    action={
                        'type': 'ir.actions.act_window',
                        'res_model': 'res.company',
                        'views': [(False, 'form')],
                        'res_id': payment.company_id.id,
                        'name': _('Compañía'),
                        'view_mode': 'form',
                    },
                    button_text=_('Editar compañía'),
                )

            # Validamos que el código de alícuota se encuentre entre 1 y 83 si no aplica CDI
            if not payment.sire_aplica_cdi:
                if int(payment.sire_codigo_alicuota) > 83:
