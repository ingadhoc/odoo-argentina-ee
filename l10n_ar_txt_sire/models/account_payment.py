...
class AccountPayment(models.Model):
    ...
    sire_aplica_cdi = fields.Boolean(readonly=False,
                                     help='Campo para archivo txt Ganancias SIRE. Marcar si aplica CDI',
                                     store=True)
    sire_aplica_acrecentamiento = fields.Boolean(readonly=False,
                                                 help='Campo para archivo txt Ganancias SIRE. Marcar si aplica CDI',
                                                 store=True)
    sire_codigo_alicuota = fields.Char(readonly=False,
                                       size=4,
                                       store=True)
    ...
    def _compute_sire_fields(self):
        ...
        """
        tag_tax_sire = self.env['account.account.tag'].search([('name', '=', 'Sire'),
                                                               ('applicability', '=', 'taxes'),
                                                               ('country_id', '=', self.env.ref('base.ar').id)])
        sire_payments = self.filtered(lambda pay: tag_tax_sire in
