...
class TestPaymentWithholdingSettlement(TestArWithholdingArRi):
        ...
        payment = self.env["account.payment"].create(
            {
                ...
                # The invoice date comes from the core fixture, and the rates of setUpClass are anchored
                # to it: a literal here would break the test whenever that fixture moves.
                "date": invoice.date,
    ...
    def test_reassign_wizard_offers_the_settlement_with_a_gap(self):
        ...
        wizard = self.env["l10n_ar.tax.settlement.reassign"].create({"line_ids": [Command.set(new_line.ids)]})
        self.assertIn(settlement, wizard.candidate_move_ids, "the settlement left with a gap is offered")
        self.assertEqual(wizard.settlement_move_id, settlement, "and comes filled in as the best guess")
        wizard.action_reassign()

        self.assertEqual(new_line.tax_settlement_move_id, settlement)

    def test_guessed_settlement_is_asked_instead_of_written(self):
        """What the entry kept is written on its own; a guess only reaches the wizard."""
        payment = self._create_posted_payment_with_withholding("2-105")
        self._settle_and_edit(payment, {"payment_reference": "edited"})
        payment.move_id.unlinked_tax_settlement_ids.unlink()
        new_line = self._get_withholding_line(payment)

        action = new_line.action_reassign_tax_settlement()

        self.assertEqual(action.get("res_model"), "l10n_ar.tax.settlement.reassign")
        self.assertFalse(new_line.tax_settlement_move_id, "nothing is linked back without the user confirming")

    def test_reassign_to_a_settlement_without_that_gap_is_refused(self):
        """A settlement that adds up is not missing this withholding: linking it there declares it twice."""
        payment = self._create_posted_payment_with_withholding("2-106")
        self._settle_and_edit(payment, {"payment_reference": "edited"})
        other_payment = self._create_posted_payment_with_withholding("2-107")
        other_settlement = self._get_withholding_line(other_payment).button_create_tax_settlement_entry()

        new_line = self._get_withholding_line(payment)
        wizard = self.env["l10n_ar.tax.settlement.reassign"].create(
            {"line_ids": [Command.set(new_line.ids)], "settlement_move_id": other_settlement.id}
        )
        with self.assertRaises(UserError):
            wizard.action_reassign()
        self.assertFalse(new_line.tax_settlement_move_id)
