...
class AccountJournal(models.Model):
    ...
    settlement_tax = fields.Selection(
        selection_add=[
            ...
            (
                "iibb_aplicado_arba_desde_01122025",
                "TXT Perc/Ret IIBB aplicadas ARBA desde 01/12/2025: Percepciones ( excepto actividad 29, 7 quincenal, 7 y 17 de Bancos)",
            ),
            (
                "iibb_aplicado_arba_act_7_desde_01122025",
                "TXT Perc/Ret IIBB aplicadas ARBA desde 01/12/2025: Percepciones Act. 7 método Percibido (quincenal)",
            ),
            # ('other', 'Other')
        ]
    )
    ...
    def iibb_aplicado_arba_desde_01122025_files_values(self, move_lines, act_7=None):
        ...
        for line in move_lines:
            ...
            if not alicuot:
                raise ValidationError(
                    _('No hay alicuota configurada en el partner "%s" (id: %s)') % (partner.name, partner.id)
                )
            content += "%05.2f" % alicuot
            # este es para el primer tipo de la especificación
