...
class AccountPayment(models.Model):
    ...
    def action_reassign_tax_settlement(self):
        """Link the withholdings back to the settlement where they were declared.

        The ones we can identify are reassigned right away; the rest go to the wizard so that the user
        picks the settlement.
        """
        self.ensure_one()
        lines = self._get_unlinked_tax_settlement_lines()
        if not lines:
            raise UserError(_("This payment has no withholdings pending reassignment."))
        reassigned = self.env["account.move.line"]
        for line in lines:
            if settlement := line._get_tax_settlement_to_reassign():
                line.reassign_tax_settlement(settlement)
                reassigned |= line
        if pending := lines - reassigned:
            return pending.action_reassign_tax_settlement()
        return reassigned._get_tax_settlement_reassign_notification()
