...
class AfipActivity(models.Model):
    ...
    def menor_alicuota(self, actividades_con_alicuota_cero):
        ...
        activities_with_aliquots = activities.filtered(
            lambda x: x.alicuota_general or x.posee_tasa_cero or x.no_posee_certificado_tasa_cero
        )
        ...
        if elementos_no_en_ambas:
            raise UserError(
                "Hay actividades en la factura que no tienen alícuota. Actividades: %s"
                % (",".join(elementos_no_en_ambas))
            )
