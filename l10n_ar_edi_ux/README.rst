...
* **Invoice PDF Legend:** Allows configuring, from the accounting settings, a legend to be printed below the Document Type letter on the Invoice PDF of documents with letter A and M: "Payment on Informed CBU" or "Operation Subject to Withholding" (ARCA RG 5762/2025). It supersedes the *Add Withholding Legend to Invoice PDF* checkbox of l10n_ar_edi, which is removed from the settings and whose value is moved into the new selector on install/upgrade, so the legend is configured in a single place and printed once. Branches with no legend of their own print the one configured on the closest parent company. Backport of odoo/enterprise#102032, incorporated upstream in l10n_ar_edi from 20.0 on: to be reverted when migrating to that version
...
3. **Invoice PDF Legend:** Go to Accounting > Settings > Argentinian Localization
   ...
   * On branches, leave it empty to print the legend configured on the parent company
