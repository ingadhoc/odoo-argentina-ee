...
class AccountCheckToDateReportWizard(models.TransientModel):
    ...
    def _get_checks_on_hand(self, journal_id, to_date):
        """
        Hacemos una query que:
        * toma todos los cheques hasta cierta fecha en una tabla t
        ...
        * a esta query le unimos otra query que tome los casos de delivered third check (a traves de las tablas t3/t4)
        ...
        SELECT * FROM (
            ...
            FROM
                ...
                LEFT JOIN
                (
                    SELECT DISTINCT ON (c.id) c.id as check_id, ap_check_op.id as payment_id, ap_check_op_move.date as operation_date, apm.code as operation_code, pair_apm.code as paired_code, c.current_journal_id as journal
                    FROM l10n_latam_check c
                    LEFT JOIN account_payment AS ap_check_op ON ap_check_op.id = c.payment_id
