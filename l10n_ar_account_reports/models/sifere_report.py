...
class L10n_ArSifereReportHandler(models.AbstractModel):
        ...
        txt_export_button = [
            {
                ...
                "sequence": 30,
                "action": "export_file",
                "action_param": "sifere_ret_perc_txt",
            ...
            },
            {
                ...
                "sequence": 30,
                "action": "export_file",
                "action_param": "nc_sifere_ret_perc_txt",
                "file_export_type": "TXT",
            },
        ]
    ...
    def sifere_ret_perc_txt(self, options):
        return {
            "file_name": "Perc/Ret IIBB SIFERE Aplicadas.txt",
            "file_content": self._sifere_book_get_txt_files(options),
            "file_type": "txt",
        }

    def nc_sifere_ret_perc_txt(self, options):
        return {
            "file_name": "NC Perc/Ret IIBB SIFERE Aplicadas.txt",
            "file_content": self._sifere_book_get_txt_files(options, refund=True),
            "file_type": "txt",
        }

    def _sifere_book_get_txt_files(self, options, refund=False):
        """Returns SIFERE txt content"""
        move_lines = self._sifere_book_get_txt_lines(options, refund=refund)
        return "".join(self._get_sifere_txt_content(move_lines)).encode("ISO-8859-1", "ignore")

    def _sifere_book_get_txt_lines(self, options):
    ...
    def _get_sifere_txt_content(self, move_lines, refund=False):
