...
patch(AccountReportFilters.prototype, {
    ...
    selectDateFilter(periodType, _reload = false) {
        ...
        // Only send a period offset update when the periodType has an entry in
        // dateFilter (i.e. it's a navigable period like year/month/etc.).
        // Non-navigable types like 'custom' and 'today' don't have an offset,
        // so we skip the period call and let the filter call carry the reload.
        const offsetPeriod = Object.prototype.hasOwnProperty.call(this.dateFilter, periodType);
        this.filterClicked({ optionKey: "date.filter", optionValue: newFilter, reload: !offsetPeriod && reload });
        if (offsetPeriod) {
     ...
     * Ensure 'calendar_year' is handled explicitly, and fall back to the
     * bare periodType (e.g. 'custom', 'today') when there is no offset entry
     * — matching core's `else { return periodType; }` branch.
     */
    getDateFilter(periodType) {
        ...
        if (offset < 0) return `previous_${periodType}`;
        return periodType;
    },
