...
#. Saca la exportación a CSV del Libro Mayor del gate de sucursales ("seleccioná la compañía principal y todas sus sucursales"). Era el único botón de exportación del reporte sin ``branch_allowed``, así que el mismo reporte con la misma selección exportaba a XLSX y se negaba a exportar a CSV.
...
#. Agrega el campo "Balance in GL" en el tablero de contabilidad (diarios de banco, efectivo y tarjeta de crédito), con el saldo contable de la cuenta por defecto del diario a la fecha de hoy. El saldo se suma **por cuenta y por compañía**: cuando dos compañías comparten la misma cuenta por defecto —el caso de las sucursales que usan la cuenta de la madre— cada diario muestra el saldo de su propia compañía y no el consolidado de todas.
